...
const NextHoliday = () => {
  ...
  const nextDaysLeft = next ? getDaysLeft(next.dateInAD) : null;

  return (
    <div className="border-border h-full w-full max-w-sm rounded-xl border bg-white p-4 shadow-sm flex flex-col">
      ...
      {next && nextDaysLeft ? (
        ...
        <div className="flex-1 flex items-center gap-2 justify-center">
          <p className="text-sm text-slate-400">Oops! No upcoming holidays.</p>
        </div>
      )}
    </div>
  );
